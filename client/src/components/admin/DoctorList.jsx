...
// M5 FIX: added loggedInUser + the same self-delete guard UserList.jsx already
// had. Previously an admin who was also seeded as a doctor record (or any
// future workflow where the two overlap) could delete their own account from
// this list with no protection at all.
const DoctorList = ({ doctors, loggedInUser, onEdit, onDelete, onSetAvailability, onResetPassword }) => {
    return (
        <div className="overflow-y-auto h-64 border rounded-lg">
            <table className="min-w-full bg-white">
                ...
                <tbody>
                    {doctors.map(doctor => {
                        const isSelf = loggedInUser && loggedInUser._id === doctor._id;
                        return (
