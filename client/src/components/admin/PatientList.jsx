...
// M5 FIX: added loggedInUser + self-delete guard, matching UserList.jsx.
const PatientList = ({ patients, loggedInUser, onEdit, onDelete, onResetPassword }) => {
    return (
            ...
            <table className="min-w-full bg-white">
                ...
                <tbody>
                    {patients.map(patient => {
                        const isSelf = loggedInUser && loggedInUser._id === patient._id;
                        return (
                            <tr key={patient._id} className="hover:bg-gray-50">
                                <td className="py-2 px-4 border-b">{patient.name}</td>
                                <td className="py-2 px-4 border-b">{patient.email}</td>
                                <td className="py-2 px-4 border-b whitespace-nowrap">
                                    <button onClick={() => onEdit(patient)} className="text-blue-500 hover:text-blue-700 p-1 rounded-full inline-flex items-center">
                                    ...
                                    <button onClick={() => onResetPassword(patient)} className="text-yellow-500 hover:text-yellow-700 p-1 rounded-full inline-flex items-center ml-2">
                                    ...
                                    <button
                                        ...
                                        className={`p-1 rounded-full inline-flex items-center ml-2 ${isSelf ? 'opacity-50 cursor-not-allowed text-gray-400' : 'text-red-500 hover:text-red-700'}`}
                                        disabled={isSelf}
